...
import { ArrowUp, Calendar, Mail, Sparkles, Briefcase, Wrench, Heart } from "lucide-react";
...
export function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-24 pb-16 bg-grid overflow-hidden">
      {/* Bottom-right glow */}
      ...
      <div className="relative z-10 max-w-2xl mx-auto w-full flex flex-col items-center gap-5">
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-5xl font-extrabold text-center tracking-tight text-white"
        >
          ...
          <div className="flex flex-wrap gap-2 p-4 pb-3">
            {suggestions.map((s) => (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-primary-900/50 text-sm text-white/80 hover:text-white hover:border-primary-500/30 hover:bg-primary-800/50 transition-all duration-200"
              >
                <s.icon size={14} className="text-glow-cyan/70" />
                {s.label}
              </button>
            ))}
          </div>
          ...
          <div className="px-4 pb-4">
            <div className="flex items-center gap-2 rounded-xl bg-bg-deep/70 border border-white/[0.06]">
              <textarea
                ...
                rows={1}
                className="flex-1 bg-transparent text-white placeholder:text-text-muted text-sm px-4 py-3.5 resize-none outline-none max-h-24"
              />
              <button
